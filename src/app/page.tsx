...
interface SidebarRightProps extends React.ComponentProps<typeof Sidebar> {devicename: string;
  timeRange: DateRange;
  ...
  return (
    <Sidebar collapsible="none" className="sticky hidden lg:flex top-0 h-svh border-l" {...props}>
      ...
      <SidebarContent>
        ...
        />
        <button  className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4  hover:border-blue-500 rounded"onClick={() => updateTimeRange({ from: new Date(), to: new Date() })}>Aktualizovat</button>
      </SidebarContent>
      <SidebarFooter>
